    ...
    fetch(`<?php echo APP_URL; ?>/menu/get-menu-item/${id}`)
        ...
        .then(data => {
            if (data.success) {
                ...
                const menuItemGroupHidden = document.getElementById('menuItemGroupHidden');
                ...
                if (menuItemGroup) menuItemGroup.value = item.group_id;
                if (menuItemGroupHidden) menuItemGroupHidden.value = item.group_id;
                if (menuItemModule) menuItemModule.value = item.module_id || '';
                if (menuItemIcon) menuItemIcon.value = item.icon;
                if (menuItemParent) menuItemParent.value = item.parent_id || '';
                if (menuItemSortOrder) menuItemSortOrder.value = item.sort_order;
                if (menuItemIsParent) menuItemIsParent.checked = item.is_parent == 1;
                if (menuItemIsActive) menuItemIsActive.checked = item.is_active == 1;
                
                // Control "Is Parent Menu" checkbox based on parent selection
                const parentValue = menuItemParent ? menuItemParent.value : '';
                ...
                const isChecked = isParentCheckbox ? isParentCheckbox.checked : false;
                const moduleSelect = menuItemModule;
                const requiredAsterisk = document.querySelector('.module-required-asterisk');
...
function addMenuItem() {
    ...
    if (menuItemIsParent) {
        menuItemIsParent.disabled = false;
    }
    
    // Reset Module dropdown state
    if (menuItemModule) {
        menuItemModule.disabled = false;
        menuItemModule.required = true;
    }
    if (requiredAsterisk) {
        requiredAsterisk.style.display = 'inline';
    }
    ...
    if (menuItemGroup) menuItemGroup.value = '<?php echo $selected_group_id; ?>';
    if (menuItemGroupHidden) menuItemGroupHidden.value = '<?php echo $selected_group_id; ?>';
    <?php endif; ?>
    
    // Update hidden input when disabled select changes
    if (menuItemGroup) {
        menuItemGroup.addEventListener('change', function() {
            if (menuItemGroupHidden) {
                menuItemGroupHidden.value = this.value;
            }
    ...
    const menuItemParent = document.getElementById('menuItemParent');
    if (menuItemParent) {
        menuItemParent.addEventListener('change', function() {
    ...
    if (menuItemIsParent) {
        menuItemIsParent.addEventListener('change', function() {
    ...
    function updateParentDropdown(groupId) {
        ...
        const currentValue = parentSelect ? parentSelect.value : '';
        
        // Clear existing options except "No Parent"
        if (parentSelect) {
            const options = parentSelect.querySelectorAll('option:not(:first-child)');
        ...
        if (groupId) {
            ...
            .then(data => {
                if (data.success && data.parentItems && parentSelect) {
                    ...
                    if (currentValue && parentSelect.querySelector(`option[value="${currentValue}"]`)) {
                    ...
                    if (parentValue && parentValue !== '' && isParentCheckbox) {
                        isParentCheckbox.checked = false;
                        isParentCheckbox.disabled = true;
                    } else if (isParentCheckbox) {
                        isParentCheckbox.disabled = false;
                    }
